...
def main(args):
    ...
    catboost_ALL = cb.CatBoostClassifier()
    catboost_ALL.load_model(args.catboost_all_model_path)

    catboost_NOHLA = cb.CatBoostClassifier()
    catboost_NOHLA.load_model(args.catboost_nohla_model_path)

    catboost_HLAONLY = cb.CatBoostClassifier()
    catboost_HLAONLY.load_model(args.catboost_hlaonly_model_path)
    ...
    ALL_test = list(catboost_ALL.predict_proba(T1D_ALL)[:, 1])
    HLA_test = list(catboost_HLAONLY.predict_proba(T1D_HLA)[:, 1])
    nonHLA_test = list(catboost_NOHLA.predict_proba(T1D_nonHLA)[:, 1])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--catboost_all_model_path', required=True, help='Path to the CatBoost ALL model file')
    parser.add_argument('--catboost_nohla_model_path', required=True, help='Path to the CatBoost NOHLA model file')
    parser.add_argument('--catboost_hlaonly_model_path', required=True, help='Path to the CatBoost HLAONLY model file')
